  ...
  <view class="recharge-page">
    ...
    <view class="fee-wrap">  
      <view class="page-container">
        ...
        <view class="amount-list">
          ...
          >
            ¥{{ item.value }}
            <image v-if="selected_amount === item.value" class="icon-choosed" src="@/static/images/svg/a19.svg" mode="aspectFit" />
          </view>
        </view>
    ...
    <view class="bottom-bar">
      <button 
        class="pay-btn" 
        :disabled="!canPay"
        @click="handleShowPayComfirmPopup"
      >
        立即支付
      </button>
    </view>
    ...
    <!-- 支付确认弹窗 -->
    <PagePopup 
      v-model:visible="is_show_comfirm_pay_popup" 
      title="缴费信息"
    >
      <view class="custom-amount">
      ...
      <view class="flex mb-40 fz-26 my-30">
        ...
        />
        <view class="ml-10 flex-1 color-999"
          @click="is_agree = !is_agree"
        >
          我已核实<text class="color-primary">收费人物业管理处、收费项目</text>的真实性，并阅读了
          <text @click="handleNavToAgreement" class="color-primary">《服务协议》</text>，我同意支付费用。
        </view>
      </view>

      <view slot="footer" class="pb-30">
    ...
    <!-- 支付弹窗 -->
    <PagePopup 
      v-model:visible="is_show_pay_popup" 
      title="缴费信息"
    >
      ...
      <view slot="footer" class="py-30 flex gap-30">
        <view class="flex-1">
...
const handleCustomAmount = () => {
  ...
  canPay.value = !!custom_amount.value && !isNaN(Number(custom_amount.value)) && parseFloat(custom_amount.value) > 0
}
